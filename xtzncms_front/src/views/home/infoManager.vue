...
<template>
<div>
      <Button type="primary" icon="plus-round" @click='insertBtn' class="btnGp">发布</Button>
      ...
      <Modal v-model='modalFlag' :title='modalTitle' @on-visible-change='ovc'>
        <Form ref='infoForm' :model='formItem' class='f' :label-width='80' :rules='ruleValidate' >
          ...
          <div slot='footer'>
          <Button type='primary' :loading='loadingStatus' v-show='modalType==1' @click='okMed' >提交表单</Button>
...
export default {
  ...
  methods: {
    ...
    getData (page, limit) {
      this.loading = true
      this.$api.get('info/page?page=' + page + '&limit=' + limit + '&searchType=' + this.searchType + '&searchValue=' + this.searchValue, null, r => {
        if (r.rspCode === 800) {
    ...
    okMed () {
      this.$refs['infoForm'].validate(fg => {
        if (fg) {
          return new Promise((resolve, reject) => {
            ...
            axios.post(this.baseUrl + 'info', param, config)
              .then(r => {
    ...
    delete (row) {
      this.$api.delete('info?uid=' + row.uid, null, r => {
        if (r.rspCode === 800) {
