...
<div>
      ...
      <Modal v-model='modalFlag' :title='modalTitle' @on-visible-change='ovc'>
        <Form ref='communityForm' :model='formItem' class='f' :label-width='80' :rules='ruleValidate' >
          ...
          <FormItem label='展示图名称' prop='imageName'>
              <Input v-model='formItem.imageName' placeholder='请填写展示图片名称' :readonly='modalType==2'></Input>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    getData (page, limit) {
      this.loading = true
      this.$api.get('community/page?page=' + page + '&limit=' + limit + '&searchType=' + this.searchType + '&searchValue=' + this.searchValue, null, r => {
        if (r.rspCode === 800) {
    ...
    okMed () {
      this.$refs['communityForm'].validate(fg => {
        if (fg) {
          return new Promise((resolve, reject) => {
            ...
            param.append('imageName', this.formItem.imageName)
            ...
            axios.post(this.baseUrl + 'community', param, config)
              .then(r => {
    ...
    delete (row) {
      this.$api.delete('community?uid=' + row.uid, null, r => {
        if (r.rspCode === 800) {
