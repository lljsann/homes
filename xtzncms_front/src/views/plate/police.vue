...
<div>
      ...
      <Modal v-model='modalFlag' :title='modalTitle' @on-visible-change='ovc'>
        <Form ref='policeForm' :model='formItem' class='f' :label-width='80' :rules='ruleValidate' >
...
export default {
  ...
  methods: {
    ...
    getData (page, limit) {
      this.loading = true
      this.$api.get('police/page?page=' + page + '&limit=' + limit + '&searchType=' + this.searchType + '&searchValue=' + this.searchValue, null, r => {
        if (r.rspCode === 800) {
    ...
    okMed () {
      this.$refs['policeForm'].validate(fg => {
        if (fg) {
          return new Promise((resolve, reject) => {
            ...
            axios.post(this.baseUrl + 'police', param, config)
              .then(r => {
    ...
    formInit () {
      ...
      this.$refs['policeForm'].resetFields()
    },
    delete (row) {
      this.$api.delete('police?uid=' + row.uid, null, r => {
        if (r.rspCode === 800) {
