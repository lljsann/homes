...
<div>
      ...
      <Modal v-model='modalFlag' :title='modalTitle' @on-visible-change='ovc'>
        <Form ref='companystructForm' :model='formItem' class='f' :label-width='80' :rules='ruleValidate' >
          ...
          <FormItem label="架构图" v-show='modalType==1'>
...
export default {
  ...
  methods: {
    ...
    getData (page, limit) {
      this.loading = true
      this.$api.get('companystruct/page?page=' + page + '&limit=' + limit, null, r => {
        if (r.rspCode === 800) {
    ...
    okMed () {
      this.$refs['companystructForm'].validate(fg => {
        if (fg) {
          return new Promise((resolve, reject) => {
            if (this.file == null || this.formItem.fileList.length === 0) {
            //  reject('图片必选')
            } else {
              resolve('success')
            }
            ...
            axios.post(this.baseUrl + 'companystruct', param, config)
              .then(r => {
    ...
    formInit () {
      ...
      this.$refs['companystructForm'].resetFields()
    },
    delete (row) {
      this.$api.delete('companystruct?uid=' + row.uid, null, r => {
        if (r.rspCode === 800) {
